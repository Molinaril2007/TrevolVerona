...
-controllare che i comuni di inizio e di fine non siano direttamente confinanti (in teoria ok)
...
public class Main {
    public static void main(String[] args) throws Exception {
        //View vista = new View("TravleVerona");
        ...
        do {
            S = comuni.get(rand.nextInt(comuni.size()));
            D = comuni.get(rand.nextInt(comuni.size()));
        ...
        List<Integer> shortestPath = ShortestPathBFS.shortestDistance(graph, S.getId(), D.getId(), V);
        ...
        View vista = new View("Trevol Verona", S, D, shortestPath.size()-2);
        Controller controller = new Controller(vista, comuni, scelte);
    }
}
